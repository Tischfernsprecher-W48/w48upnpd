...
//	if ( (ret = UpnpInit(intIpAddress,0) ) != UPNP_E_SUCCESS)
	if ( (ret = UpnpInit(intIpAddress, port) ) != UPNP_E_SUCCESS)
	{
	...
	sprintf(descDocUrl, "http://%s:%d/%s", UpnpGetServerIpAddress(), UpnpGetServerPort(), g_vars.descDocName);
